export output_dir_name="proftpdChgrp"
...
sleep 8
send_cmnd "site chgrp 777 /tmp/file.txt"
rm /tmp/file.txt
$pftpstop
...
############################################################# 
touch /tmp/file.txt
$tracer -f "chmod" -i "file0" -r 1 -o $traces_path -- $program -n -c /tmp/PFTEST/PFTEST.conf &
sleep 8
send_cmnd "site chmod 777 /tmp"
rm /tmp/file.txt
$pftpstop
...
sleep 20
send_cmnd "site chgrp 777 /tmp"
rm /tmp/file.txt
$pftpstop

touch /tmp/file.txt
$rpiden -d $output_path --  $program -n -c /tmp/PFTEST/PFTEST.conf &
sleep 20
send_cmnd "site chgrp 777 /tmp"
rm /tmp/file.txt
$pftpstop